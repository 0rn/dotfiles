...
-- turn off history! = could set delete_check_events instead
luasnip.config.set_config({history=false,})

-- Diagnostic popup configuration
vim.o.updatetime=250
-- don't auto open diagnostic
...
local fn = {
   ...
   -- expand snippet, select next completion, jump
    superTab = function(fallback)
      if luasnip.expandable() then
          luasnip.expand()
      elseif cmp.visible() then
          cmp.select_next_item()
      elseif luasnip.jumpable(1) then
          luasnip.jump(1) --jump forward
      else
          fallback()
      end
   end,
   shiftSuperTab = function(fallback)
      if cmp.visible() then
...
cmp.setup({
   ...
   matching = {
    ...
    mapping = cmp.mapping.preset.insert({
      ...
      -- insert completion
      ['<CR>']        = cmp.mapping.confirm(),--fn.complete,

      -- Snippet Expand and Suggestion Menu Navigation
      ['<Tab>']       = fn.superTab,
      ['<S-Tab>']     = fn.shiftSuperTab,
      ...
      -- Cancel completion
      ['<Esc>']     = fn.closeAndThrough, -- reset to normal mode
